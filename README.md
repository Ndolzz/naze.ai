...
- **Bug kritis ikut diperbaiki:** `api/chat.js` sebelumnya mengunci model ke `gemini-3.6-flash` — id model itu tidak ada di lineup Gemini, jadi setiap request akan gagal di sisi API. Sudah dikembalikan ke `gemini-2.5-flash` (sesuai yang didokumentasikan di bagian atas README ini), yang juga mendukung Google Search grounding.
