...
## Update: respons terpotong saat pakai provider cadangan — diperbaiki (root cause, bukan CSS)
- **Penyebab asli** bukan soal tinggi container atau timeout idle (itu bug yang sudah pernah diperbaiki, lihat update di atas) — penyebabnya adalah `finish_reason`/`finishReason` dari provider. Kalau Gemini/Groq/OpenRouter/Mistral berhenti karena kehabisan jatah `max_tokens` di tengah jawaban, mereka tetap mengirim sinyal "selesai" yang sah secara teknis (`finishReason: "MAX_TOKENS"` di Gemini, `finish_reason: "length"` di provider OpenAI-compatible) — tapi jawabannya sendiri belum selesai. Kode lama menutup stream begitu melihat *finish reason apa pun*, jadi jawaban yang kepotong token limit dikirim ke user seolah-olah sudah lengkap.
- **Perbaikan:** `api/chat.js` sekarang membedakan "selesai beneran" (`STOP` / `stop`) dari "kepotong limit" (`MAX_TOKENS` / `length`). Kalau kepotong limit, backend otomatis mengirim permintaan lanjutan ke provider yang sama (bawa jawaban sejauh ini + instruksi "lanjutkan, jangan mengulang"), lalu menyambung hasilnya ke stream SSE yang sama — sampai 3x percobaan lanjutan (`MAX_CONTINUATIONS`). Frontend cukup terus menerima `content_block_delta` seperti biasa, plus notifikasi kecil "Naze melanjutkan respons..." yang muncul sebentar di bawah bubble saat ini terjadi.
- Batas token per request (`MAX_TOKENS_CAP`) juga dinaikkan dari 4096 → 8192, supaya lanjutan otomatis di atas jarang diperlukan untuk jawaban yang wajar panjangnya.
- Parser SSE kedua provider (Gemini & OpenAI-compatible) juga dibuat lebih tahan banting: satu chunk yang gagal di-parse (JSON rusak/parsial) tidak lagi menghentikan seluruh respons — cuma dilewati, lanjut ke chunk berikutnya.
