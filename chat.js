...
export default async function handler(req) {
  ...
  if (!imagePresent) {
    ...
    for (const provider of shuffle(FALLBACK_PROVIDERS)) {
      ...
      const fbPayload = { model: provider.model, messages: baseMessages, max_tokens: cappedTokens, stream: true };
      let upstream = null;
      for (const apiKey of keys) {
        try {
          const res = await fetch(provider.url, {
...
function streamOpenAICompatibleResponse(upstream, providerLabel, groundingExtra) {
  const reader = upstream.body.getReader();
  const decoder = new TextDecoder();
  const encoder = new TextEncoder();
  let buf = '';
